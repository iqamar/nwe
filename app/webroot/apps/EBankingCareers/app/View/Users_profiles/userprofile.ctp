  ...
  <div class="profile-user userprofile-form">
    ...
    <?php if ($userRec['users_profiles']['photo']) {
			if (file_exists(MEDIA_PATH.'/files/user/logo/'.$userRec['users_profiles']['photo'])) {
				...
				echo $this->Html->image('/img/nophoto.jpg',array('style'=>''));
			}
		 } 
		 else {
	   		echo $this->Html->image('/img/nophoto.jpg',array('style'=>''));
			...
			?>
				<div class="activity-icon">
					<?php echo $this->Html->image(MEDIA_URL.'/files/starsign/'.$starsign__Row['Star_sign']['icon'],
																							array('onClick'=>'showStarSign('.$id.','.$friend_id.')'),
                ...
                <ul>
                    ...
                    <li>
                    	<span class="smalltext">
                         <a href="javascript:showStarSign('<?php echo $id?>','<?php echo $friend_id?>')" class="poplight smalltext" style="text-decoration:none; color:#C70000;">
         ...
         <?php if ($uid != $friend_id) { 
				...
				<?php	if (empty($checkRequest)){?>
                     ...
                     <span id="connectUser_<?php echo $friend_id?>">
					  ...
					  ?> 
                       <a href="#users_recommended_text_for_user" class="recommendation">Recommendation</a>
                      <?php }?>
...
<?php } if ($getTotalUser) {?>
 ...
 <div class="profile-box">
    ...
    <div class="profile-box-heading-rgt">
 	<a href="Javascript:sharedConnection('<?php echo $uid?>','<?php echo $friend_id?>','all');">All<?php $total_con = sizeof($getTotalUser); if($total_con) echo '('.$total_con.')';
 	?>
    </a>
 ...
 <div id="sendmessage" class="popup_block">
                    <!--your content start-->
                        ...
                        <form action="/users_profiles/user_send_message/" method="post" class="userprofile-form">
                        	<?php if($this->Session->read('email')){ $yourEmail = $this->Session->read('email');}?>
		  ...
		  <div class="wholike-rgt">
			  <ul>
				  <li>
                      ...
                      <?php if ($user_id != $uid) {?>
                      	<?php if ($flag == true) {?>
                      	<input name="Connect" type="button" id="Connect" value="Following" class="red-bttn" onclick="followers('<?php echo $follow_id;?>','0','<?php echo $user_id;?>','<?php echo $uid;?>','followers')">
                      	<?php }
							else {
						?>
                        <input name="Connect" type="button" id="Connect" value="Follow" class="red-bttn" onclick="followers('<?php echo $follow_id;?>','2','<?php echo $user_id;?>','<?php echo $uid;?>','followers')">
                        <?php }}?>
                      </div>
					  <h1>
                      	<a href="/users_profiles/userprofile/<?php echo $follower__row['users_profiles']['user_id'];?>">
