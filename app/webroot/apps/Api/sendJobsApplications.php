...
$query = "SELECT * FROM jobs_applications WHERE api_status = 0 ORDER BY id limit 0,20";
...
exit;
echo		$json_fields_client='{"api_key":"'.MAILER_API_KEY.'","email_details":{"fromname":"'.fullescape(MAILER_REGISTER_FROMNAME).'","subject":"'.fullescape(MAILER_JOB_APPLICATION_CLIENT_SUBJECT. urldecode($jTitle)).'","from":"'.MAILER_REGISTER_FROMEMAIL.'","replytoid":"'.MAILER_REGISTER_REPLYTO.'","content":"'.fullescape($strBody).'"},"settings":{"template":"848"},"recipients":["'.$cEmail.'"],"attributes":{"NAME":["'.$name.'"],"EMAIL":["'.$email.'"],"JOB_NAME":["'.$jTitle.'"],"CLIENT_NAME":["'.$cTitle.'"]}';
			 if(!empty($cv)){
			...
			$ch1 = curl_init();
                        ...
                        curl_setopt($ch1,CURLOPT_POSTFIELDS, array('data' => $json_fields_client));

                        $result1 = curl_exec($ch1);
                        curl_close($ch1);

			if(($result=="success") || ($result1=="success")){
				$mysqli->query("UPDATE jobs_applications SET  api_status = 1 WHERE id ='".$appid."'");
				echo "done";
			}
