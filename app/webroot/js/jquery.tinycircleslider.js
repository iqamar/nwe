...
{   
    ...
    $.tiny.circleslider = {
        ...
        ,   callback     : '' // function that executes after every move
    };
	...
	function SetprofilesDetails(obj){
	alert(obj);
	};	
    function Slider( root, options )
    {
       ...
       function gotoClosestSlide()
        ...
        function setCSS( angle, bFireCallback )
        {
			...
			if(bFireCallback ){
				//alert();
				strstr = (oLinks[ iCurrent ].title).split("||");
				$("#profileDescName").html(strstr[0]);
				$("#profileDescJobTitle").html(strstr[1]);
				$("#profileDescLocation").html(strstr[2]);
