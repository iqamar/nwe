<div class="close_icon_row" style="width:567px;">
				<div class="close_icon_row_left" style="width:548px;">Favorited  by users </div><div class="close_icon" style="float:right;" onclick="close_PopUp_Favorite('<?php echo $tweet_id;?>')">
				</div>
			</div>
           ...
           <?php echo $this->Html->image('loading.gif');?>
 			...
 			<?php foreach ($user_favorite_tweets as $user_favorite_row) {
						$user_id = $user_favorite_row['users_profiles']['user_id']; ?>
						<div style="clear:both; height:60px; padding:12px; margin-bottom:5px; border: 1px dashed #ccc;">
						<?php if ($user_favorite_row['users_profiles']['photo']){
							echo $this->Html->image('/files/users/'.$user_favorite_row['users_profiles']['photo'],array(
							...
							<div style="width:415px; float:left; margin-left:5px;">
								<h3 style="font-size:12px; font-weight:bold; color:#333;">
									<?php echo $user_favorite_row['users_profiles']['firstname']." ".$user_favorite_row['users_profiles']['lastname'];?>
                   					 <?php if ($user_favorite_row['users_profiles']['handler']) {?>
                    				<span style="font-size:12px; color:#8D8D8D;">
                    					<?php echo " @".$user_favorite_row['users_profiles']['handler'];?>
                   					 </span>	<?php }?>
                               </h3>
                               <?php $followers_counts = ''; $j = 1; 
								 foreach ($user_following as $followers__rows) {
			if ($followers__rows['Users_following']['following_id'] == $user_favorite_row['users_profiles']['user_id'] && $followers__rows['Users_following']['status'] ==2) { 
										$followers_counts += $j;
										$j++; }
								} ?>
            ...
            <a href="javascript:close_PopUp_Favorite('<?php echo $tweet_id;?>');" class="savebtn" style="float:right;">Cancel</a>