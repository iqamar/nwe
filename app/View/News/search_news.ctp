  <?php if ($get_news_search) {
  		foreach ($get_news_search as $news_inCat_Row) {
				...
				$auther_fullname = $news_inCat_Row['users_profiles']['firstname']." ".$posts_inCat_Row['users_profiles']['lastname'];
				$pub_profile = $news_inCat_Row['users_profiles']['handler'];
		...
		<div class="blogslisitng" id="<?php echo $newsId;?>">
			...
			<div class="blogslisitng-pic">
				...
				if ($news_inCat_Row['news']['image_url']) {
					...
					echo $this->Html->link($this->Html->Image(MEDIA_URL.'/files/news/original/'.$news_inCat_Row['news']['image_url']),'#',array('escape'=>false)); 
				} else {
	...
	<?php } } 
		...
		echo "<div class='error_msg'>Sorry we did not find news against your search..</div>";
	}?>