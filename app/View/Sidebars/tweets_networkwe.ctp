...
<div class="greybox">
		...
		foreach ($get_tweets as $tweet__Row) {
					...
					$tweet_author = $tweet__Row['users_profiles']['photo'];
			?>
        <div class="rgtwidget-listing2">
				<div class="rgtwidget-listing2-logo">
					<a href="/users_profiles/userprofile/<?php echo $tweet__Row['users_profiles']['user_id']?>">
						<?php if ($tweet_author && file_exists(MEDIA_PATH.'/files/user/icon/'.$tweet_author)) {
			...
			<div class="rgtwidget-listing2-rgt">
				<ul>
					...
					<li>
                    	<?php 
						$tweet_owner = $tweet__Row['tweets']['user_id'];
						if ($tweet_owner != $uid) {
							$flage = false;
