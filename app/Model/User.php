...
class User extends AppModel {
    ...
    public function user_login($email, $password) {
        ...
        $isAccountInDB = ClassRegistry::init('User')->find('first', array(
            'conditions' => array('email' => $email, 'password' => $password)));
        if ($isAccountInDB != '') {

            return $isAccountInDB;
        } else {
            return 0;
        }
    }
    ...
    public function saveData($loginid) {

        $this->request->data['Users_profile']['user_id'] = $loginid;
        if (ClassRegistry::init('users_profiles')->save($this->request->data['Users_profile']['user_id'])) {
            $res = "saved";
        } else {
            $res = "notsaved";
        }
