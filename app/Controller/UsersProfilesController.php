        ...
        if ($this->userInfo['users']['id']) {
            ...
            if ($this->request->is('post')) {
               ...
               $isCompanyInDB = $this->Company->find('all', array('fields' => array('Company.id'), 'conditions' => array("title like '%" . $company_title . "%'"), 'limit' => 1));
                ...
                $this->request->data['Users_experience']['responsibilities'] = $this->request->data['Users_profile']['responsibilities'];
                ...
                if (ClassRegistry::init('Users_experience')->save($this->request->data)) {
					...
					/* user new job adding start*/
				
				if ($this->request->data['Users_experience']['end_date'] == 'Present') {
					
					$user_company = $this->Company->find('first', array('fields' => array('Company.title'), 'conditions' => array('Company.id='.$company_id)));
					
					$company_name = $user_company['Company']['title'];
					
					$designation = $this->request->data['Users_profile']['designation'];
					...
					$user_name = $this->Session->read('fullname');
					
					$user_name = trim($user_name);
					
					$user_picture = $this->Session->read('picture');
					
					//$user_data = $this->getCurrentUser($uid);
					$user_info = $designation; if($company_name) { $user_info .= " at ".$company_name; }
					
					$user_description = $user_name."|".$user_info;
					
					$this->request->data['Statusupdate']['user_id'] = $uid;
					
					$this->request->data['Statusupdate']['user_text'] = $user_description;
					
					$this->request->data['Statusupdate']['photo'] = $user_picture;
					
					$this->request->data['Statusupdate']['content_type'] = "jobchange";
					
					$this->request->data['Statusupdate']['share_with'] = 1;
					
					$this->request->data['Statusupdate']['share'] = 0;
					
					$this->request->data['Statusupdate']['update_shared'] = 0;
					
					$this->request->data['Statusupdate']['update_type'] = 1;
					
					$this->request->data['Statusupdate']['created'] = $created_dt;
					
					$this->request->data['Statusupdate']['modified'] = $created_dt;
    ...
    public function load_exp() {
        if ($this->request->is('ajax')) {
            ...
            $json = $this->Users_experience->find('first', array(
                ...
                'fields' => array(
                    'Users_experience.id', 'Users_experience.user_id', 'Users_experience.location', 'Users_experience.designation', 'Users_experience.responsibilities',
            ...
            if ($this->request->data) {
                ...
                if ($image_type == true) {
                    ...
                    if ($this->Users_profile->save($this->request->data)) {
                        ...
                        if ($this->User_profile_strength->updateAll(array('photo' => $strength_photo), array('User_profile_strength.user_id' => $uid))) {
...
//	$this->set(compact('roles'));
    ...
    public function userprofile() {
        if ($this->userInfo['users']['id']) {
            ...
            if ($this->request->is('get')) {
				...
				$email_display = $this->getUserSettings($id);
				$this->set('email_display',$email_display);
                ...
                $uSers_exp[] = ClassRegistry::init('users_experiences')->find('all', array('fields' => array(
                        ...
                        'users_experiences.designation',
                        'users_experiences.end_date',
                        'users_experiences.responsibilities',                        
                        'users_experiences.location',
                        'companies.id',
                        'companies.title',
                        'companies.logo'
		...
		$user_connections= sizeof($commonUserArray);
        if (sizeof($commonUserArray) > 1) {
            $commonUserArray = @implode(',', $commonUserArray);
            $getTotalUser = ClassRegistry::init('users_profiles')->find('all', array('limit' => 10, 'conditions' => array('users_profiles.user_id IN(' . $commonUserArray . ')')));
        } else {
            $getTotalUser = ClassRegistry::init('users_profiles')->find('all', array('limit' => 10, 'conditions' => array('users_profiles.user_id' => $commonUserArray[0])));
        }
        $this->set('getTotalUser', $getTotalUser);
		$this->set('user_connections',$user_connections);
        ...
        $birthdate = $uers_p['users_profiles']['birth_date'];
        ...
        if (( $month == 3 && $day > 20 ) || ( $month == 4 && $day < 20 )) {
            $condition = array('Star_sign.name = "Aries"');
        } elseif (( $month == 4 && $day > 19 ) || ( $month == 5 && $day < 21 )) {
            $condition = array('Star_sign.name = "Taurus"');
        } elseif (( $month == 5 && $day > 20 ) || ( $month == 6 && $day < 21 )) {
            $condition = array('Star_sign.name = "Gemini"');
        } elseif (( $month == 6 && $day > 20 ) || ( $month == 7 && $day < 23 )) {
            $condition = array('Star_sign.name = "Cancer"');
        } elseif (( $month == 7 && $day > 22 ) || ( $month == 8 && $day < 23 )) {
            $condition = array('Star_sign.name = "Leo"');
        } elseif (( $month == 8 && $day > 22 ) || ( $month == 9 && $day < 23 )) {
            $condition = array('Star_sign.name = "Virgo"');
        } elseif (( $month == 9 && $day > 22 ) || ( $month == 10 && $day < 23 )) {
            $condition = array('Star_sign.name = "Libra"');
        } elseif (( $month == 10 && $day > 22 ) || ( $month == 11 && $day < 22 )) {
            $condition = array('Star_sign.name = "Scorpio"');
        } elseif (( $month == 11 && $day > 21 ) || ( $month == 12 && $day < 22 )) {
            $condition = array('Star_sign.name = "Sagittarius"');
        } elseif (( $month == 12 && $day > 21 ) || ( $month == 1 && $day < 20 )) {
            $condition = array('Star_sign.name = "Capricorn"');
        } elseif (( $month == 1 && $day > 19 ) || ( $month == 2 && $day < 19 )) {
            $condition = array('Star_sign.name = "Aquarius"');
        } elseif (( $month == 2 && $day > 18 ) || ( $month == 3 && $day < 21 )) {
            $condition = array('Star_sign.name = "Pisces"');
        }



        $user_starsign_dob = $this->Star_sign->find('first', array('fields' => array('
																				   Star_sign.id,
																				   Star_sign.name,
																				   Star_sign.icon
																				  '),
        ...
        $this->set('starsign__Row', $user_starsign_dob);
    ...
    public function myprofile() {
        ...
        if ($this->userInfo['users']['id']) {
			...
			$email_display = $this->getUserSettings($uid);
			$this->set('email_display',$email_display);
            ...
            $uers_p = ClassRegistry::init('users_profiles')->find('first', array('fields' => array(
                ...
                'conditions' => array('users_profiles.user_id' => $uid)
                )
            );



            $uSers_exp[] = ClassRegistry::init('users_experiences')->find('all', array(
                'fields' => array(
                    'users_experiences.start_date',
                    'users_experiences.id',
                    'users_experiences.designation',
                ...
                'conditions' => array('users_experiences.user_id=' . $uid . ' AND users_experiences.end_date ="Present"'),
                'order' => 'STR_TO_DATE(users_experiences.start_date,"%m-%Y") DESC'
                )
            );
            ...
            $total_user_experience = ClassRegistry::init('users_experiences')->find('all', array('fields' => array('users_experiences.start_date',
																												   'users_experiences.end_date',
																												   'companies.id',
																												   ),
																								 'joins' => array(
																								 ...
																								 'conditions' => array('users_experiences.user_id=' . $uid)
                )
            );
            $this->set('total_user_experience', $total_user_experience);
            $last_edu = ClassRegistry::init('users_qualifications')->find('all', array('fields' => array('
            ...
            $userHaveSkills = ClassRegistry::init('skills')->find('all', array('fields' => array(
                ...
                'joins' => array(
                    ...
                    array('alias' => 'skill_recommendations',
                        ...
                        'type' => 'left',
                        'foreignKey' => false,
                        'conditions' => array('skill_recommendations.user_id=' . $uid . ' AND skill_recommendations.skill_id = users_skills.skill_id AND skill_recommendations.recommendation=1'
                        )
                    )
                ),
                'conditions' => array('users_skills.user_id' => $uid),
        ...
        $uers_RecommendedListingwithoutAjax = ClassRegistry::init('skill_recommendations')->find('all', array('fields' => array('
            ...
            'conditions' => array('skill_recommendations.user_id=' . $uid . ' AND skill_recommendations.recommendation=1'),
            'group' => 'skill_recommendations.id'
                )
        );
        $this->set('uers_RecommendedListingwithoutAjax', $uers_RecommendedListingwithoutAjax);
        ...
        $reqUser = ClassRegistry::init('connections')->find('all', array('fields' => array('
            connections.friend_id
            '),
            'conditions' => array('connections.user_id=' . $uid . ' AND connections.request=1'
            ), 'limit' => 10
            )
        );
        //$result[]='';
        ...
        if (sizeof($result) > 1) {
            ...
            $getTotalUser = ClassRegistry::init('users_profiles')->find('all', array('limit' => 10, 'conditions' => array('users_profiles.user_id IN(' . $result . ')')));
        } else {
            $getTotalUser = ClassRegistry::init('users_profiles')->find('all', array('limit' => 10, 'conditions' => array('users_profiles.user_id' => $result[0])));
        }
        ...
        $userFollows = ClassRegistry::init('users_followings')->find('all', array('fields' => array('
																								  ...
																								  count(users_followings.user_id) as total_follow
																								  '),
            'conditions' => array('users_followings.following_id=' . $uid . ' AND users_followings.following_type="users" AND users_followings.status=2'
            )
                )
        );
        ...
        if ($userFollowYou != 0) {
            ...
            if ($this->User_profile_strength->updateAll(array('followers' => $strength_followers), array('User_profile_strength.user_id' => $uid))) {
                //$this->Session->setFlash('Strength successfully saved.');
            } else {
                echo "not saved handler strength";
            }
        } else {
            if ($this->User_profile_strength->updateAll(array('followers' => 0), array('User_profile_strength.user_id' => $uid))) {
                //$this->Session->setFlash('Strength successfully saved.');
            } else {
                echo "not saved handler strength";
            }
        ...
        if ($userFollowingsbyYou != 0) {
            ...
            if ($this->User_profile_strength->updateAll(array('following' => $strength_following), array('User_profile_strength.user_id' => $uid))) {
                //$this->Session->setFlash('Strength successfully saved.');
            } else {
                echo "not saved handler strength";
            }
        } else {
            if ($this->User_profile_strength->updateAll(array('following' => 0), array('User_profile_strength.user_id' => $uid))) {
                //$this->Session->setFlash('Strength successfully saved.');
            } else {
                echo "not saved handler strength";
            }
        ...
        $uers_following_companies = $this->Users_following->find('all', array('fields' => array('
            ...
            'conditions' => array('Users_following.user_id=' . $uid . ' AND Users_following.following_type="company" AND (Users_following.status=2 AND companies.flag="page")'
            )
                )
        );
        ...
        $user_commendations_text = ClassRegistry::init('Users_recommendation')->find('all', array('fields' => array('
            ...
            'conditions' => array('Users_recommendation.friend_id' => $uid
            )
                )
        );
        ...
        $this->set('user_commendations_text', $user_commendations_text);
        $given_recommendation = ClassRegistry::init('Users_recommendation')->find('all', array('fields' => array('Users_recommendation.id'),
            'conditions' => array('Users_recommendation.user_id' => $uid)
                )
        );
        $this->set('count_given_recommendation', sizeof($given_recommendation));
        ...
        $groupsListing = ClassRegistry::init('groups')->find('all', array('fields' => array('
            ...
            'conditions' => array('users_followings.following_type = "groups" AND users_followings.status =2 AND users_followings.user_id=' . $uid
            )
                )
        );
        ...
        $this->loadModel('Star_sign');
        //echo $birthdate = $uers_p['users_profiles']['birth_date'];
        $condition = "";
        $birthdate = $uers_p['users_profiles']['birth_date'];
        list ($year, $month, $day) = explode("-", $birthdate);
    ...
    public function user_star() {
        ...
        if ($this->request->is('get')) {
            ...
            $user_star_detail = $this->Star_sign->find('first', array('conditions' => array('Star_sign.id=' . $star_id)));
            $this->set('user_star_detail', $user_star_detail);
            ...
            $this->set('star_type', $star_type);

            $star_name = $user_star_detail['Star_sign']['name'];
            //$star_name = "'".$star_name."'";
            $current_date = date('Y-m-d');
            //$last_days = date('Y-m-d H:i:s', strtotime("now -1 days"));
            $current_date = "'" . $current_date . "'";
            //$last_days = "'".$last_days."'";
            $user_last_stars = ClassRegistry::init('daily_horoscopes')->find('all', array('fields' => array($star_name),
                'conditions' => array('DAYOFYEAR(daily_horoscopes.created) =DAYOFYEAR(' . $current_date . ')'),
                'order' => 'daily_horoscopes.created DESC'));
            $this->set('user_last_stars', $user_last_stars);
            $this->set('star_name', $star_name);
    ...
    public function upload() {
        ...
        if ($this->userInfo['users']['id']) {
            ...
            if ($this->Users_profile->updateAll(array('photo' => '"' . $filename . '"'), array('Users_profile.user_id' => $uid))) {
    ...
    public function user_send_message() {
        if ($this->request->is('post')) {
            ...
            $this->request->data['msg_inbox']['from_user_id'] = $this->request->data['user_id'];
            $this->request->data['msg_inbox']['to_user_id'] = $this->request->data['sender_id'];
            $this->request->data['msg_inbox']['subject'] = $this->request->data['subject'];
            $this->request->data['msg_inbox']['message'] = $this->request->data['message'];
            ...
            if ($this->msg_inbox->save($this->request->data)) {
				...
				$this->request->data['msg_sent']['from_user_id']= $this->request->data['user_id'];
				$this->request->data['msg_sent']['to_user_id']= $this->request->data['sender_id'];
				$this->request->data['msg_sent']['subject']= $this->request->data['subject'];
				$this->request->data['msg_sent']['message']= $this->request->data['message'];
				...
				if($this->msg_sent->save($this->request->data)){
					...
					$strBody="";
					$email = $data['reciver'];
					$json_fields='{"api_key":"'.MAILER_API_KEY.'","email_details":{"fromname":"'.$this->fullescape(MAILER_MESSAGE_USER_FROMNAME).'","subject":"'.$this->fullescape($data['subject']).'","from":"'.MAILER_MESSAGE_USER_FROMEMAIL.'","replytoid":"'.$data['sender'].'","content":"'.$this->fullescape($strBody).'"},"settings":{"template":"776"},"recipients":["'.$email.'"],"attributes":{"CONNECTION_NAME":["'.$data['friendname'].'"],"NAME":["'.$fullname.'"],"TITLE":["'.$user_Profiles['tags'].'"],"MSG_TEXT":["'.$data['message'].'"],"UNSUB_URL":["'.$this->fullescape(UNSUB_URL."?xEmail=".$email).'"]}}';
    ...
    public function invite_for_chat() {
		...
		$email = $request_user_Email['email'];
                $json_fields='{"api_key":"'.MAILER_API_KEY.'","email_details":{"fromname":"'.$this->fullescape(MAILER_CHAT_REQUEST_FROMNAME).'","subject":"'.$this->fullescape($requested_user_Profile['firstname']." ".MAILER_CHAT_REQUEST_SUBJECT).'","from":"'.MAILER_CHAT_REQUEST_FROMEMAIL.'","replytoid":"'.MAILER_CHAT_REQUEST_REPLYTO.'","content":"'.$this->fullescape($strBody).'"},"settings":{"template":"774"},"recipients":["'.$email.'"],"attributes":{"CONNECTION_NAME":["'.$requested_user.'"],"NAME":["'.$fullname.'"],"TITLE":["'.$user_deisgnation.'"],"CONNECTION_LINK":["'.$connection_link.'"],"PROFILE_LINK":["'.$profile_link.'"],"UNSUB_URL":["'.$this->fullescape(UNSUB_URL."?xEmail=".$email).'"]}}';
                $ch = curl_init();
                curl_setopt($ch,CURLOPT_URL, MAILER_SEND_API);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                curl_setopt($ch,CURLOPT_POST, true);
                ...
                curl_close($ch);
                if ($api_result == "success") {
                    //$this->Session->setFlash('Connection Request sent');
                    $this->redirect(array('controller' => 'users_profiles', 'action' => 'myprofile'));
                }
		exit;
    ...
    public function user_delete() {
        if ($this->userInfo['users']['id']) {
            ...
            if (ClassRegistry::init('Deleted_users')->save($user_deleted)) {
				...
				$db->rawQuery("DELETE FROM master_activities WHERE user_id='" . $uid . "' OR post_owner = '" . $uid . "'");
    ...
    public function wish_birthday() {
        ...
        if ($this->request->is('post')) {
            ...
            if (ClassRegistry::init('users_messages')->save($this->request->data)) {
                ...
                $strBody="";
				$email = $reciver['email'];
                $json_fields='{"api_key":"'.MAILER_API_KEY.'","email_details":{"fromname":"'.$this->fullescape(MAILER_BIRTHDAY_USER_FROMNAME).'","subject":"'.$this->fullescape(MAILER_BIRTHDAY_USER_SUBJECT).'","from":"'.MAILER_BIRTHDAY_USER_FROMEMAIL.'","replytoid":"'.MAILER_BIRTHDAY_USER_REPLYTO.'","content":"'.$this->fullescape($strBody).'"},"settings":{"template":"1076"},"recipients":["'.$email.'"],"attributes":{"CONNECTION_NAME":["'.$friendname['firstname'].'"],"NAME":["'.$yourname['firstname'].'"],"TITLE":["'.$yourname['tags'].'"],"BIRTHDAT_TEXT":["'.$birthday_text.'"],"UNSUB_URL":["'.$this->fullescape(UNSUB_URL."?xEmail=".$email).'"]}}';
