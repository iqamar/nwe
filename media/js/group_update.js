	...
	getUrl.on('input paste keyup',function() { //user types url in text field		
		...
		if (match_url.test(getUrl.val())) {
			...
			if($("img#preimg").attr('src')==''){
				...
				$.post('/groups/extractProcess',{'url': extracted_url}, function(data){         
...
function checkField(commentID) {
	var fieldValue =  document.getElementById('comments_'+commentID).value;
		if (fieldValue == ' ') {
...
function unfollow(user_follow_id,user_id,status,group_id) {
	$("#group_follow_by_user").html('<img src="http://media.networkwe.com/img/loading.gif" />');
	$.ajax({
	url     : NETWORKWE_URL+"/groups/follow_group",
	type    : "POST",
	cache   : false,
	...
	if (status == 2) {
		document.getElementById('group_follow_update').style.display='block';
	}
	else {
		document.getElementById('group_follow_update').style.display='none';	
	}
	},
	complete: function() {
		//$("#group_follow_by_user").html('');		
	},
	error : function(data) {
...
function add_group_comment(content_id) {
	
	//alert(category_title);
	var user_id = document.getElementById('user_id').value;
	var content_id = document.getElementById('content_id_'+content_id).value;
	var admin_id = document.getElementById('admin_id').value;
	var comments = document.getElementById('comments_'+content_id).value;
	$('#group_loader'+content_id).show();
	$.ajax({
	url     : NETWORKWE_URL+"/groups/add_comments",
	type    : "POST",
	cache   : false,
	data    : {user_id: user_id,content_id:content_id,comments:comments,admin_id:admin_id},
	success : function(data){
		//$('#group_comments_'+content_id).html(data);
		responseArray = data.split("::::");
		$("#total_following_"+content_id).html(responseArray[0]);
		$("#group_comments_"+content_id).html(responseArray[1]);
	},
	complete: function () {
		$('#group_loader'+content_id).hide();
		document.getElementById('comments_'+content_id).value = '';	
     },
	error : function(data) {
	$('#group_comments_'+content_id).html("there is error in your script.");
	}
	});		
}
function showLikes(content_id,like){
		$("#user_like_update_"+content_id).html('<img src="http://media.networkwe.com/img/loading.gif" style="float:left;" />');
		var user_id = document.getElementById('user_id').value;
		var content_type = document.getElementById('like_type').value;
		var created = document.getElementById('like_date_'+content_id).value;
		
		$.ajax({
		url     : NETWORKWE_URL+"/comments/add_like",
		type    : "GET",
		cache   : false,
		data    : {user_id: user_id,content_type:content_type,content_id:content_id,created:created,like:like},
		success : function(data){	
		responseArray = data.split("::");
		$("#user_like_update_"+content_id).html(responseArray[0]);
		$("#ajax_res"+content_id).html(responseArray[1]);
		},
		error : function(data) {
		$("#user_like_update_"+content_id).html("there is error");
		}
		});
}
...
function delete_update(update_id) {
	var checkstr =  confirm('Are you want to delete this?');
		if(checkstr == true){
  			$.ajax({
					url     : NETWORKWE_URL+"/groups/delete_update",
					type    : "GET",
					cache   : false,
...
function delete_comment(comment_id,content_id) {
	var checkstr =  confirm('Are you want to delete this?');
		if(checkstr == true){
  			$.ajax({
					url     : NETWORKWE_URL+"/groups/delete_comment",
					type    : "GET",
					cache   : false,
