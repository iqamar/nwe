...
(function ($) {
			...
			for (i = 1; i <= params[el.id].sph; i++) {
				...
				for (j = 1; j <= params[el.id].spw; j++) {
					...
					if (params[el.id].links) {
						$('#' + el.id).append("<a href='../../../../networkme/1/banner/" + links[el.id][0] + "' class='cs-" + el.id + "' id='cs-" + el.id + i + j + "' style='width:" + sWidth + "px; height:" + sHeight + "px; float: left; position: absolute;'></a>");
					} else {
	...
	$.fn.coinslider.defaults = {
		width: 690, // width of slider panel
		height: 334, // height of slider panel
		...
		prevText: 'prev',
		nextText: 'next',
		navigation: false, // show/hide prev, next and buttons
