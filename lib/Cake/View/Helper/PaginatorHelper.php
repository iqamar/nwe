	...
	public function numbers($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			for ($i = $start; $i < $params['page']; $i++) {
				$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i),array('class'=>'current'), $options), compact('class')) . $separator;
			}
			...
			if ($currentTag) {
				...
				$out .= $this->Html->tag($tag, $this->link( $params['page'], array('page' =>  $params['page']),array('class'=>'current active')), array('class' => $currentClass));
			}
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				if ($i == $params['page']) {
					...
					$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i),array('class'=>'current'), $options), compact('class'));
				}
				if ($i != $params['pageCount']) {
					$out .= $separator;
				}
 ...
 */
	public function first($first = '<< first', $options = array()) {
		...
		if (is_int($first) && $params['page'] >= $first) {
			...
			$out = $this->Html->tag($tag, $this->link($first, array('page' => 1),array('class'=>'current'), $options), compact('class')) . $after;
		}
		return $out;
	}
	...
	public function last($last = 'last >>', $options = array()) {
		...
		if (is_int($last) && $params['page'] <= $lower) {
			...
			$out = $before . $this->Html->tag(
				$tag, $this->link($last, array('page' => $params['pageCount']),array('class'=>'current'), $options), compact('class')
			);
		}
		return $out;
	}
