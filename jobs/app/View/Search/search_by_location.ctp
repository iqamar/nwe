...
<div class="box">
	...
	<?php 
		...
		foreach($data as $row){
			...
			echo "<li>".$this->Html->Image(MEDIA_URL.'/files/flags/'.$row['JFA']['alpha_3'].'.png',array('style'=>'width:32px;float:left;margin-right:5px'))."<div style='float:left;'>".$this->Html->link($row['JFA']['name'].'&nbsp;',array('controller'=>'search','action'=>'jobsByLocation/'.$row['JFA']['id'].'-'.$row['JFA']['name']),array('escape'=>false))."</div><span class='num'>(".$row[0]['COUNT(FA.id)']."&nbsp;Jobs)</span></li>";
