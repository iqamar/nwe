...
<div class="box">
	...
	<?php 
		...
		foreach($data as $row){
			...
			echo "<li>".$this->Html->link($row['J']['title'],array('controller'=>'search','action'=>'jobsByFunction/'.$row['J']['id'].'-'.$row['J']['title']),array('escape'=>false,'style'=>'max-width:280px;height:30px;overflow:hidden;display:block;float:left;'))."<span class='num'>(".$row[0]['COUNT(FA.id)']."&nbsp;Jobs)</span></li>";
